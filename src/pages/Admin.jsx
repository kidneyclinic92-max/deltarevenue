...
function SectionEditor({ sectionId, config, updateSection }) {
  ...
  if (!data) return <p className="text-primary-400">Unknown section</p>

  const set = (key, value) => updateSection(sectionId, { [key]: value })
  const setAll = (obj) => updateSection(sectionId, () => obj)
  ...
  switch (sectionId) {
    ...
    case 'header': {
        ...
        const setLinks = (next) => updateSection('header', { navLinks: next })
        return (
          <>
            ...
            {links.map((link, i) => (
              <div key={i} className="mb-2 flex gap-2">
                ...
                <input value={link.href || ''} onChange={(e) => setLinks(links.map((l, j) => (j === i ? { ...l, href: e.target.value } : l)))} placeholder="href (#approach)" className="flex-1 rounded-lg border border-primary-600 bg-primary-900 px-4 py-2 text-primary-100" />
                <button type="button" onClick={() => setLinks(links.filter((_, j) => j !== i))} className="text-red-400 px-2">×</button>
              </div>
            ))}
            <button type="button" onClick={() => setLinks([...links, { label: '', href: '#' }])} className="text-accent-gold hover:text-accent-goldLight text-sm">+ Add nav link</button>
          </>
        )
      }
    case 'deltaApproach':
    ...
    case 'trust':
      return (
        <>
          ...
          <p className="text-primary-300 text-sm mb-2">Paragraphs (HTML allowed, e.g. &lt;strong&gt;)</p>
    ...
    case 'specialties':
      return (
        <>
          <p className="text-primary-300 text-sm mb-2">Specialty options (one per item)</p>
    ...
    case 'services': {
      ...
      const updateItem = (i, f) => set('items', items.map((it, idx) => (idx === i ? (typeof f === 'function' ? f(it) : { ...it, ...f }) : it)))
        ...
        <>
          ...
          {items.map((item, i) => (
            <div key={i} className="mb-4 p-4 rounded-lg border border-primary-600 space-y-2">
              ...
              <Field label="Icon (emoji)" value={item.icon} onChange={(v) => updateItem(i, { ...item, icon: v })} />
              <Field label="Title" value={item.title} onChange={(v) => updateItem(i, { ...item, title: v })} />
              <Field label="Description" value={item.desc} onChange={(v) => updateItem(i, { ...item, desc: v })} multiline />
              <Field label="Link (href)" value={item.href} onChange={(v) => updateItem(i, { ...item, href: v })} />
            </div>
          ))}
    ...
    case 'footer': {
        ...
        const setLinks = (next) => updateSection('footer', { links: next })
        return (
          <>
            <Field label="Logo text" value={data.logoText} onChange={(v) => set('logoText', v)} />
            <Field label="Logo highlight" value={data.logoHighlight} onChange={(v) => set('logoHighlight', v)} />
            ...
            {links.map((link, i) => (
              <div key={i} className="mb-2 flex gap-2">
                <input value={link.label || ''} onChange={(e) => setLinks(links.map((l, j) => (j === i ? { ...l, label: e.target.value } : l)))} placeholder="Label" className="flex-1 rounded-lg border border-primary-600 bg-primary-900 px-4 py-2 text-primary-100" />
                <input value={link.href || ''} onChange={(e) => setLinks(links.map((l, j) => (j === i ? { ...l, href: e.target.value } : l)))} placeholder="href" className="flex-1 rounded-lg border border-primary-600 bg-primary-900 px-4 py-2 text-primary-100" />
                <button type="button" onClick={() => setLinks(links.filter((_, j) => j !== i))} className="text-red-400 px-2">×</button>
              </div>
            ))}
...
export default function Admin() {
  ...
  return (
    <div className="min-h-screen bg-primary-900">
      {/* Top bar: View site, Export, Reset, Log out */}
      ...
      {/* Single page: all sections stacked - full width, spacious */}
      <main className="w-full max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-10 pb-20 flex flex-col items-center text-center">
        ...
        {SECTIONS.map((s) => (
          <section key={s.id} id={`admin-${s.id}`} className="w-full mb-14">
            <h2 className="text-xl font-semibold text-accent-gold mb-4">{s.label}</h2>
